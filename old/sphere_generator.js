(function () {
    ...
    class TextureMap {
        constructor(length, smoothing = true) {
            ...
            this.ERPtoCubeFace = this.initProgram(this.ERPtoCubeFaceVertex, this.ERPtoCubeFaceFragment);
        ...
        loadFromEquirectangular(image) {
            ...
            const faceLocation = gl.getUniformLocation(this.ERPtoCubeFace, 'face');
            for (let i = 0; i < 6; i++) {
        ...
        loadFromFaces(north, west, south, east, up, down) {
            ...
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_POSITIVE_Y, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, rotate(west, Math.PI));
            gl.texImage2D(gl.TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, rotate(east, 0));
            gl.texImage2D(
        ...
        renderSphereSides(alpha = 0, beta = 0, gamma = 0) {
            ...
            const rotationLocation = gl.getUniformLocation(this.CubeMapToHemisphere, 'rotation');
            const euler = new THREE.Euler(alpha, beta, gamma, 'XYZ');
            const quaternion = new THREE.Quaternion().setFromEuler(euler);
            gl.uniform4f(rotationLocation, ...quaternion.toArray());

            const angleLocation = gl.getUniformLocation(this.CubeMapToHemisphere, 'angle');
        ...
        renderAnimationAtlas(beginningRotation, endingRotation, timeCurve, frameCount) {
            const gl = this.gl;
            this.loadProgram(this.CubeMapToHemisphere);
            ...
            sides.forEach((side) => {
                ...
                [side.canvas.width, side.canvas.height] = [this.length, this.length * frameCount];
                side.ctx = side.canvas.getContext('2d');
            });

            const valueAt = (target) => {
                const x = (t) => timeCurve[0] * t * 3 * (1 - t) ** 2 + timeCurve[2] * t * t * 3 * (1 - t) + t * t * t;
                const y = (t) => timeCurve[1] * t * 3 * (1 - t) ** 2 + timeCurve[3] * t * t * 3 * (1 - t) + t * t * t;
                let bounds = {
                    lower: 0,
                    upper: 1,
                    get center() {
            ...
            const angleLocation = gl.getUniformLocation(this.CubeMapToHemisphere, 'angle');
            const beginning = new THREE.Quaternion().setFromEuler(new THREE.Euler(...beginningRotation, 'XYZ'));
            const ending = new THREE.Quaternion().setFromEuler(new THREE.Euler(...endingRotation, 'XYZ'));
            //console.log(beginning, ending);

            frameCount--;
            for (let frame = 0; frame <= frameCount; frame++) {
                const rotation = beginning.clone().slerp(ending, frame / frameCount);
                gl.uniform4f(rotationLocation, ...rotation.toArray());

                for (let i = 0; i < 4; i++) renderHemisphere(sides[i].ctx, frame, 0, (i * Math.PI) / 2);
    ...
    Plugin.register('sphere_generator', {
    ...
    function createMenuAction() {
        menuAction = new Action('generate_sphere', {
    ...
    function vectorInput(label, id, values, minimums = [], maximums = [], step = 1) {
    ...
    function VectorInput({ label, id, inputs = {} }) {
        ...
        <div class="dialog_bar form_bar form_bar_center">
            ${makeLabel(label)}
            <div id="${id}" class="dialog_vector_group half">${inputStrings.join('')}</div>
        </div>`;
    }

    function NumberInput({ label, id, value, onchange = '', min = 1, max = undefined, step = 1 }) {
        return /*html*/ `
    ...
    function selectInput(label, id, value, options, onchange = '') {
        const optionElements = Object.entries(options)
            .map(
                ([id, displayName]) =>
                    /*html*/ `<option value="${id}" ${id == value ? 'selected' : ''}>${displayName}</option>`
            )
            .join('');
        ...
        new Dialog({
            ...
            lines: [
                SelectionInput({
                    ...
                    onChange: () => {
                        for (let option of this.options) {
                            const section = document.getElementById(`${option.value}-input`);
                            section.style.display = option.selected ? 'block' : 'none';
                        }
                    },
                    options: [
                        { displayName: 'Equirectangular', id: 'equirectangular' },
                ...
                Wrapper({
                    ...
                    style: {
                        display: defaultMapping === 'equirectangular' ? 'block' : 'none',
                    },
                ...
                Wrapper({
                    ...
                    children: SIDES.reduce((raw, side) => raw + imageInput(`${side} face`, `${side}-face`), ''),
                }),
                `<hr>`,
                CheckboxInput({
                    ...
                    animateByDefault,
                    onChange: () => {
                        document.getElementById('animation-input').style.display = this.checked ? 'block' : 'none';
                    },
                }),
                VectorInput({
                    label: 'Rotation',
                    id: 'rotation',
                    inputs: defaultRotation.map((rotation) => ({ value: rotation })),
                }),
                Wrapper({
                    id: 'animation-input',
                    style: {
                    ...
                    children: [
                        ...
                        NumberInput({ label: 'Frame Count', id: 'frames', value: defaultFrameCount }),
                        SelectionInput({
                            ...
                            options: [
                                ...
                                { id: 'easeInOut', displayName: 'Ease In And Out' },
                                { id: 'bezier', displayName: 'Cubic Bezier' },
                            ],
                            onChange: () => {
                                const bezierInput = document.getElementById('bezier-input');
                                bezierInput.style.display = this.value == 'bezier' ? 'block' : 'none';
                            },
                        }),
                        Wrapper({
                ...
                NumberInput({
                    ...
                    onChange: () => {
                        ...
                        document.getElementById('element-count').textContent =
                            (3 * detail * (detail - 1)) / 2 + 1 + ' elements will be generated.';
                    },
                }),
                /*html*/ `
    ...
    async function getFormData() {
        ...
        const getVectorValues = (id) =>
            [...document.getElementById(id).children].map((input) => parseFloat(input.value));
        const isChecked = (id) => document.getElementById(id).checked;

        data.mapping = getValue('mapping');
        data.texture_length = parseInt(getValue('texture-length'));
        const texture_inputs = data.mapping == 'cubemap' ? SIDES.map((side) => `${side}-face`) : ['texture'];
        data.textures = await Promise.all(texture_inputs.map((input) => readImageInput(input)));

        data.animate = isChecked('animate');
        data.rotation = getVectorValues('rotation').map((i) => (i * Math.PI) / 180);
        data.rotateTo = getVectorValues('rotate-to').map((i) => (i * Math.PI) / 180);
    ...
    async function generateSphere(data) {
        if (!data) data = await getFormData();
        let elements = [];
        ...
        const sideData = data.animate
            ? textureMap.renderAnimationAtlas(data.rotation, data.rotateTo, data.time_curve, data.frames)
        ...
        pointsOfSubdividedOctant(data.detail).forEach((point) => {
            const corner = point.clone().multiply(new THREE.Vector3().fromArray(data.size.map((i) => i / 2)));
                ...
                new Cube({
                    ...
                    faces: Object.fromEntries(
                        SIDES.map((side, index) => [
                            side,
                            new Face(side, { uv: getUV(side, point), texture: sides[index].uuid }, this),
                        ])
                    ),
                })
                    .addTo(sphereGroup)
    ...
    function pointsOfSubdividedOctant(n) {
        ...
        for (let face = 0; face < 3; face++) {
            ...
            for (let i = 0; i < n; i++) {
                for (let j = 0; j < n; j++) {
                    const corner1 = new THREE.Vector3(...getCoords(i, j)).normalize();
                    const corner2 = new THREE.Vector3(...getCoords(i + 1, j + 1)).normalize();
